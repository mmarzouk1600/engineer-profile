        ...
        altEditor.prototype,
        {
            ...
            _setup: function () {
                ...
                var modal = '<div class="modal fade altEditor-modal reveal" id="' + modal_id + '" tabindex="-1" role="dialog" data-reveal>' +
                    ...
                    '<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="' + this.language.modalClose + '">' +
                    '</button>' +
                    '</div>' +
            ...
            _initLanguage: function () {
                this.language.modalClose = this.language.modalClose || 'اغلاق';
                this.language.edit = this.language.edit || {};
                this.language.edit = { 
                    title: this.language.edit.title || 'تعديل',
                    button: this.language.edit.button || 'تعديل'
                };
                this.language.delete = this.language.delete || {};
                this.language.delete = { 
                    title: this.language.delete.title || 'حذف',
                    button: this.language.delete.button || 'حذف' 
                };
                this.language.add = this.language.add || {};
                this.language.add = { 
                    title: this.language.add.title || 'اضافة',
                    button: this.language.add.button || 'اضافة'
                };
                this.language.success = this.language.success || 'Success!';
                this.language.error = this.language.error || {};
                this.language.error = { 
                    message: this.language.error.message || 'حدث خطأ غير معروف!',
                    label: this.language.error.label || 'خطأ!',
                    responseCode: this.language.error.responseCode || 'Response code: ',
                    required: this.language.error.required || 'الحقل مطلوب',
                    unique: this.language.error.unique || 'حقل مكرر'
                };
            },
            ...
            _openDeleteModal: function () {
                ...
                var selector = this.modal_selector;
                var fill = function () {
                    var btns = '<button type="button" class="btn btn-secondary" data-bs-dismiss="modal">' + that.language.modalClose + '</button>' +
                    ...
                    $(selector).find('.modal-body').html(that.language.deleteMessage || `<h5>هل أنت متأكد من ازالة  ${adata.count()} من الصفوف ؟</h5>`);
            ...
            */
            createDialog: function(columnDefs, modalTitle, buttonCaption, closeCaption, buttonClass, formName) {
                ...
                var fill = function () 
                {
                    var btns = '<button type="button" class="btn btn-secondary" data-bs-dismiss="modal" data-close>' + closeCaption + '</button>' 
